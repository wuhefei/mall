...
import com.atguigu.gulimall.auth.util.AuthCodeGenerater;
...
public class LoginController {
    ...
    public R sendCode(@RequestParam("phone") String phone) {
       ...
       // String code = UUID.randomUUID().toString().substring(0, 5);
        String code = AuthCodeGenerater.getCode();
        // 手机号号和验证码存入redis  5分钟过期
        String redisStr = code + "_" + System.currentTimeMillis();
        stringRedisTemplate.opsForValue().set(AuthServerConstant.SMS_CODE_CACHE_PREFIX + phone, redisStr,30, TimeUnit.MINUTES);
    ...
    public String login(UserLoginVo vo, RedirectAttributes redirectAttributes,
        ...
        if (r.getCode() == 0) {
            ...
            session.setAttribute(AuthServerConstant.LOGIN_USER, memberRespVo); //使用springsession放入reids中中 reids会自动续期的哦
    ...
    /**
     * 解决不支持get请求的问题，特别注意 注释掉GulimallWebConfig.java中的视图映射。
     * @param session
     * @return
     */
